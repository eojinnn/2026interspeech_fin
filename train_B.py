...
def main(args):
    ...
    while not stop_training:
        ...
        epoch_count += 1
            ...
            # epoch 100부터 frontend prior delta unfreeze
        if epoch_count == 100:
            if hasattr(model, 'module'):
                model.module.set_frontend_prior_trainable(True)
            else:
                model.set_frontend_prior_trainable(True)
            logger.info('Unfroze frontend prior delta at epoch {}'.format(epoch_count))
        # 训练
