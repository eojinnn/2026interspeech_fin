...
class AuditoryFrontend(nn.Module):
    def __init__(
        ...
        # residual prior:
        # mel: high freq 쪽 약간 +, low freq 쪽 약간 -
        # gcc: mel과 반대
        self.register_buffer("mel_prior_log", 0.35 * prior_curve)
        self.register_buffer("gcc_prior_log", -0.35 * prior_curve)

        # Summary network: [log_power, coherence, velocity] -> hidden
        ...
        self.max_log_weight = 0.40   # exp(±0.7) ~= [0.50, 2.01]
    ...
    def get_weights(self, stft, mag):
        ...
        mel_prior = self.mel_prior_log.unsqueeze(0).expand(B, -1)
        gcc_prior = self.gcc_prior_log.unsqueeze(0).expand(B, -1)
