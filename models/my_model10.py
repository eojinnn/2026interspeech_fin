...
class AuditoryFrontend(nn.Module):
    ...
    def forward(self, wav):
        # wav shape: [Batch, Mics(4), Samples(T * 480)] 
        # 주의: 외부에서 프레임 단위로 미리 자르지 말고 원본 파형을 넘기세요.
        B, M, F, T = wav.shape
        ...
        mag = torch.abs(stft) ** 2
        mel = torch.matmul(mag.transpose(-1, -2), self.mel_filters).transpose(-1, -2)
        ...
        for m1 in range(M):
            for m2 in range(m1 + 1, M):
                ...
                X2 = stft[:, m2, :, :]
                R = X1 * torch.conj(X2)
                R = R / (torch.abs(R) + 1e-8) # PHAT Weighting
                
                # IFFT로 lag 도메인 변환
                cc = torch.fft.irfft(R, n=self.n_fft, dim=1)
